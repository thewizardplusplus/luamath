...
-- BoundingBox:expand()
function TestBoundingBox.test_expand_scalar()
  local box = BoundingBox:new(Vector2D:new(3, 4), Vector2D:new(7, 8))

  local result = box:expand(2)
...
function TestBoundingBox.test_expand_vector()
  local box = BoundingBox:new(Vector2D:new(3, 4), Vector2D:new(7, 8))
...
function TestBoundingBox.test_expand_invalid()
  ...
  luaunit.assert_error_msg_contains(
    "`min` must be at most `max` on each axis",
    function()
      box:expand(Vector2D:new(-10, -10))
    end
  )
end

-- BoundingBox:scale()
function TestBoundingBox.test_scale_scalar()
  local box = BoundingBox:new(Vector2D:new(0, 0), Vector2D:new(10, 20))

  local result = box:scale(2)
...
function TestBoundingBox.test_scale_vector()
  local box = BoundingBox:new(Vector2D:new(0, 0), Vector2D:new(10, 20))
...
function TestBoundingBox.test_scale_to_point()
  ...
  local result = box:scale(Vector2D:new(0, 0))
...
function TestBoundingBox.test_scale_rejects_negative_scalar()
  local box = BoundingBox:new(Vector2D:new(0, 0), Vector2D:new(10, 20))
